...
| --- | --- | --- |
| `analyzer` | `"simple"` | How text is split into terms. `simple`: Unicode word segmentation, lowercase, diacritics folded to ASCII (`Café` → `cafe`), no stemming. `english`: the same, then English **stop words** removed (the Snowball list — `the`, `of`, `my`, `is`, `don't`, … 174 function words that would otherwise dominate BM25 ranking) and English stemming (Snowball/Porter2), so `camera`, `cameras` and `Camera's` are one term and `run` finds `running` — on indexed text and on query terms alike, phrases included. Use `english` for prose in English; `simple` for identifiers, codes, mixed-language text, or when [prefix terms](queries.md#full-text-search--matchesfield-query-k-n) must complete character by character (a stemmed index holds stems, so `securit*` does not find `security`, stored as `secur`). The analyzer is part of the index; changing it — or upgrading to a release that changes what an analyzer does, as adding stop words to `english` did — rebuilds the index. |
...
Only the first **65 536 words** of a value are indexed; text past that is stored but not searchable. The words a create or update will index count against the query governor's examined-rows budget (an update, once per object it changes), so an oversized value is refused rather than indexed. This cap arrived with a change to what both analyzers do, so an index built by an earlier release is rebuilt in the background on the first open after upgrading.
