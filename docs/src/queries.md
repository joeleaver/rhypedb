...
**Under security rules** a search ranks only the documents the caller may read. Each candidate is checked against the `read` rule first; the top-`k` cut, the prefix-expansion cap and the BM25 statistics (document frequency, document count and average length) are then computed over the readable candidates alone, so neither a score, nor the number of rows under `k`, nor a "prefix matches too many terms" error depends on documents the caller cannot see. Scores therefore differ from the same query run without rules — they are comparable within one result, not across callers. More generally, with rules on every step of a query works only on rows the caller may read: a `.filter`, `.limit`, traversal or write after a search or filter never sees an unreadable row (a write over unreadable matches is a no-op, not a permission error). A write straight after `Type.get(id)` is still judged by the write rule alone, so deleting an object you may delete but not read works when you name it. One thing rules do not hide: resource limits are charged for the work a query really does, and that includes reading index entries and rows the caller then cannot see, so whether a query is refused for exceeding the examined-rows budget can reflect how many such rows exist — a coarse count, never their contents (the refusal names the configured limit, not how much of it was used).
