...
- **Full-text search is built in.** Mark a `String` field `@fulltext` and query it with `.matches(.field, "invoice 4471", k: 20)` — BM25-ranked, with required terms, phrases, prefix terms (`camera*`) and an optional English stemmer, maintained inside each object's transaction next to your vector index.
